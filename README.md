...
I believe OpenWRT/DD-WRT are the best solution to address security concerns related to the router bans for security conscious consumers on existing and/or EOL hardware. While it still requires a degree of security mindset from the end-user, OpenWRT 25.12 has made great progress to `security by default` in their design. While it does still utilize unknown proprietary firmware blobs, OpenWRT sandboxes them to the best of it's abilities. Given that it replaces unknown proprietary manufacturer firmware with auditable open source code it is my opinion that this is the best way for the end-user to resolve the security concerns addressed by the FCC for existing and EOL hardware.
