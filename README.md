...
## THE SOLUTION TO ROUTER BANS:
I believe OpenWRT/DD-WRT are the solution to address security concerns related to the router bans for security conscious consumers. While it still requires a degree of security mindset from the end-user, OpenWRT 25.12 has made great progress to `security by default` in their design. Given that it replaces unknown proprietary manufacturer firmware with auditable open source code it is my opinion that this should be how the end-user resolves the security concerns addressed by the FCC for existing hardware.
